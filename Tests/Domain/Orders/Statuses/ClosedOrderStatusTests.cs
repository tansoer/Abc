...
namespace Abc.Tests.Domain.Orders.Statuses {
    ...
    [TestClass]
    public class ClosedOrderStatusTests :OrderStatusesTests<ClosedOrderStatus, OrderStatus> {

        protected override ClosedOrderStatus createObject() {
            var d = random<OrderStatusData>();
            d.OrderStatus = OrderLifecycleStatus.Closed;
            return OrderStatusFactory.Create(d) as ClosedOrderStatus;
        }
